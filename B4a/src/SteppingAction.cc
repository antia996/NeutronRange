...
// Collect energy and track length step by step
  ...
  if (volume == fDetConstruction->GettrackerPhys() ) {
      fEventAction->AddTracker(edep, stepLength);
  }
  ...
  if (particle->GetPDGEncoding() == 2112) {
      ...
      /*
      else if (volume == fDetConstruction->GetBackDetectorPhys()) {
          G4double x_back = step->GetPreStepPoint()->GetPosition().x(); 
          G4double y_back = step->GetPreStepPoint()->GetPosition().y(); 

          auto analysisManager = G4AnalysisManager::Instance();
          analysisManager->FillH2(1, x_back, y_back);
      }*/
  }
