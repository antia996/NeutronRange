...
namespace B4a
{
    ...
    void SteppingAction::UserSteppingAction(const G4Step* step)
    {
        ...
        if (step->GetTrack()->GetDefinition()->GetPDGCharge() == 0. && particle->GetPDGEncoding() == 2112 && trackID == 1) {
            ...
            if (volume == fDetConstruction->GethormigonPV()) {
                fEventAction->AddHormigon(edep, stepLength);
            }

            if (volume == fDetConstruction->GetBackDetectorPhys()) {
                fEventAction->AddBackDetector(edep, stepLength);
        ...
        if (particle->GetPDGEncoding() == 2112 && trackID == 1) {
            if (volume == fDetConstruction->GetDetectorPhys()) {
                G4double x = step->GetPreStepPoint()->GetPosition().x();
                G4double y = step->GetPreStepPoint()->GetPosition().y();
