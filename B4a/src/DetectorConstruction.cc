...
G4VPhysicalVolume* DetectorConstruction::DefineVolumes()
{
  ...
  //Creo el volumen de hormigon

  G4double hormigon_hx = 20 * cm;
  G4double hormigon_hy = 30.0 * cm;
  G4double hormigon_hz = 20 * cm;

  G4Box* hormigonBox = new G4Box("ColumnaHormigon", hormigon_hx, hormigon_hy, hormigon_hz);

  G4LogicalVolume* hormigonLog
      = new G4LogicalVolume(hormigonBox, hormigon, "ColumnaHormigon");
  ...
  //Agujereo la columna de hormigón con agua
  G4double Rinit = 0.0;
  G4double Rfin = 40 * mm;
  G4double altura = hormigon_hy;
  ...
  // Creo el detector:
  // 
  G4double detector_hx = 20.0 * cm;
  G4double detector_hy = 30.0 * cm;
  G4double detector_hz = 20.0 * cm;

  G4Box* detectorBox = new G4Box("Detector", detector_hx, detector_hy, detector_hz);
  G4LogicalVolume* detectorLog
      = new G4LogicalVolume(detectorBox, NaI, "Detector");
  ...
  G4double Backdetector_hx = 20.0 * cm;
  G4double Backdetector_hy = 30.0 * cm;
  G4double Backdetector_hz = 20.0 * cm;
  ...
  G4LogicalVolume* BackdetectorLog
          = new G4LogicalVolume(BackdetectorBox, NaI, "BackDetector");


  //Coloco el detector justo detrás de el cilindro
  G4double BackdetectorPos_x = 0.0 * m;
  G4double BackdetectorPos_y = 0.0 * m;
  G4double BackdetectorPos_z = +50.0 * cm;
  ...
  worldLog->SetVisAttributes(G4VisAttributes::GetInvisible());
    ...
    G4VisAttributes* hormigonVisAtt = new G4VisAttributes(G4Colour(0.0, 1.0, 0.0));// Green
    hormigonVisAtt->SetVisibility(true);
    hormigonLog->SetVisAttributes(hormigonVisAtt);
    
    G4VisAttributes* WaterVisAtt = new G4VisAttributes(G4Colour(1.0, 1.0, 0.0));// Green
    WaterVisAtt->SetVisibility(true);
    WaterTubeLV->SetVisAttributes(WaterVisAtt);
    ...
    G4VisAttributes* BackdetectorVisAtt = new G4VisAttributes(G4Colour(1.0, 0.0, 0.0)); // White
